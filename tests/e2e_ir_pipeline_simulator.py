...
def simulate_siem_detection(event):
    ...
    time.sleep(0.5) # Simulate pipeline transmission delay
    
    # Check rule logic match
...
def trigger_soar_containment(alert):
    ...
    try:
        response = requests.post(SOAR_WEBHOOK_URL, json=alert, timeout=2)
        if response.status_code == 200:
            print(f"  [SUCCESS] SOAR Response: {response.json().get('status', 'Isolated')}")
            return True
        else:
            print(f"  [SIMULATED] Webhook unreachable ({response.status_code}). Simulating isolation fallback.")
            return True
...
def run_pipeline():
    ...
    if alert:
        contained = trigger_soar_containment(alert)
        end_time = time.time()
